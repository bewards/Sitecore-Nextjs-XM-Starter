...
function Update-EnvFileFromTemplate() {
    # update .env.local and .env files from templates (.tpl)
    Push-Location .\..\..\src\rendering
    $envLocalTplFile = ".env.local.tpl"
    $envLocalBackupFile = ".env.local.bak"
    $envLocalFile = ".env.local"

    if (Test-Path $envLocalFile) {
        Copy-Item -Force .\$envLocalFile .\$envLocalBackupFile
    }
    Copy-Item -Force .\$envLocalTplFile .\$envLocalFile
    Pop-Location
...
# directory: run/[sitecore-xm1]
Push-Location $workingDirectoryPath

# Copy .env.tpl into .env file so that devs have latest commited properties
Update-EnvFileFromTemplate;
...
if ($ByPass) {
    dotnet sitecore login --cm https://cm.demosite.localhost/ --auth https://id.demosite.localhost/ --allow-write true --client-id "SitecoreCLIServer" --client-secret "testsecret" --client-credentials true
}
else {
    dotnet sitecore login --cm https://cm.demosite.localhost/ --auth https://id.demosite.localhost/ --allow-write true
}
...
# push the serialized items and publish
Write-Host "Pushing items to Sitecore..." -ForegroundColor Green
dotnet sitecore ser push --exclude tags:[content]
dotnet sitecore ser push --include tags:[content] --publish
# dotnet sitecore ser push --publish
if ($LASTEXITCODE -ne 0) {
    Write-Error "Serialization push failed, see errors above."
}

# [REMOVED] only needed for project initialization
...
Start-Process https://cm.demosite.localhost/sitecore/
Start-Process https://www.demosite.localhost/
