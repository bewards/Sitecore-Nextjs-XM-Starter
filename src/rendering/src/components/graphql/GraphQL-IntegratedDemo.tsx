...
const GraphQLIntegratedDemo = (props: GraphQlIntegratedDemoProps): JSX.Element => {
  ...
  return (
    <div data-e2e-id="graphql-integrated">
      ...
      <p>
        Integrated GraphQL executes GraphQL queries within the Layout Service endpoint, and merges the query results
        into the Layout Service result JSON. The query results can be seen by inspecting the Layout Service response.
      </p>
      ...
      {contextItem && (
        <div>
          ...
          id: {contextItem.id}
          <br />
          page title: {contextItem.title.value}
          <br />
          children:
          <ul>
            {contextItem.children.results.map((child: Item) => (
              <li key={child.id}>
                ...
                &nbsp; (editable title too! <Text field={child.title.jsonValue} />)
              </li>
            ))}
          </ul>
        </div>
