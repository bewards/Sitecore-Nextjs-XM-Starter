...
import { _BasePage as AppRoute, Item, GraphQlConnectedDemo as GrapQLConnectedDemoDatasource } from 'graphql-types';
...
const GraphQLConnectedDemo = (props: GraphQLConnectedDemoProps): JSX.Element => {
  ...
  return (
    <div data-e2e-id="graphql-connected">
      ...
      <p>
        Connected GraphQL executes GraphQL queries directly against the Sitecore GraphQL endpoint. This example runs the
        query server-side using component-level <code>getStaticProps</code>/<code>getServerSideProps</code>, a feature
        of the Sitecore JSS Next.js SDK. These are aggregated during the the Next.js page-level{' '}
        <code>getStaticProps</code>/<code>getServerSideProps</code> execution.
      </p>
      ...
      {props.contextItem && (
        <div>
          ...
          <br />
          page title: {props.contextItem.title?.value}
          <br />
          children:
          <ul>
            {props.contextItem.children?.results.map((child: unknown) => {
              const routeItem = child as RouteItem;

              return (
                <li key={routeItem.id}>
                  <NextLink href={routeItem.url.path}>{routeItem.title?.value}</NextLink>
                  (editable title too! <Text field={routeItem.title?.jsonValue} />)
                </li>
              );
            })}
          </ul>
