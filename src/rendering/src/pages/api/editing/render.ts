...
import { azureEditingDataService } from 'lib/editing';
 ...
 * which is set to "http://localhost:3000/api/editing/render" by default (see \sitecore\config\demosite.config).
 *
...
const handler = new EditingRenderMiddleware({
  resolveServerUrl: (req: NextApiRequest) => {
    return `${process.env.VERCEL ? 'https' : process.env.APP_ENV !== 'local' ? 'https' : 'http'}://${req.headers.host}`;
  },
  editingDataService: azureEditingDataService,
}).getHandler();
