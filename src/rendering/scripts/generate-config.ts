...
const defaultConfig: JssConfig = {
  ...
  buildOnceSites: process.env[`${constantCase('buildOnceSites')}`],
  disableSSGFetch: process.env[`${constantCase('disableSsgFetch')}`],
};
...
function generateConfig(defaultConfig: JssConfig): void {
  jssConfigFactory
    ...
    .then((config) => {
      console.log('jssConfigFactory.create(defaultConfig) result:');
      console.log(config);
