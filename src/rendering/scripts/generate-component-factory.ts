...
function getComponentList(path: string): (PackageDefinition | ComponentFile)[] {
  const components = getItems<PackageDefinition | ComponentFile>({
    ...
    // filter out storybook files as they are not components for sitecore
    return (
      !(c as ComponentFile).componentName.includes('.stories') ||
      typeof (c as PackageDefinition).components !== 'undefined'
    );
  });
